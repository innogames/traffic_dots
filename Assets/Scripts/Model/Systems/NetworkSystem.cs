...
namespace Model.Systems
{
	...
	public class NetworkSystem : ComponentSystem
	{
		...
		public void AddConnection(Entity connectionEntity)
		{
			...
			int startNetwork = -1;
			int endNetwork = -1;
			if (!_nodeData.TryGetValue(connection.StartNode, out temp)) startNetwork = AddNode(connection.StartNode);

			if (!_nodeData.TryGetValue(connection.EndNode, out temp)) endNetwork = AddNode(connection.EndNode);

			//1. if two node are in the same networks => network cache change
			if (startNetwork == endNetwork) _networkData[startNetwork].AddConnection(connectionEntity);
...
//			var agent = _agents[agentEntity];
			...
			while (_openList.Count > 0)
			{
				...
				PathData temp;
				if (_closeList.TryGetValue(min.Node, out temp)) continue;
		...
		public Entity FollowPath(Entity agentEntity)
		{
			...
			int agentNodeLvl = agentNode.Networks.Length - 1;
